...
def check_for_alerts(df):
    ...
    if alert_messages:
        ...
        print("✅ حالة الطقس غداً مستقرة in جميع المحافظات.")
...
def run_weather_pipeline():
    try:
        ...
        # 2. صب وجدولة البيانات الـ 3 في الداتابيز مع تنظيف التكرارات مسبقاً
        print("☁️ حفظ جدول الـ Daily (تنظيف التكرار ثم الإضافة)...")
        with engine.connect() as conn:
            for _, row in final_daily.iterrows():
                conn.execute(
                    text("DELETE FROM daily_weather WHERE city = :city AND date = :date"),
                    {"city": row['city'], "date": row['date']}
                )
            conn.commit()
        final_daily.to_sql('daily_weather', engine, if_exists='append', index=False)

        print("⏳ حفظ جدول الـ Hourly التفصيلي (تنظيف التكرار ثم الإضافة)...")
        with engine.connect() as conn:
            for _, row in final_hourly.iterrows():
                conn.execute(
                    text("DELETE FROM hourly_weather WHERE city = :city AND time = :time"),
                    {"city": row['city'], "time": row['time']}
                )
            conn.commit()
        final_hourly.to_sql('hourly_weather', engine, if_exists='append', index=False)

        print("⚡ حفظ جدول الـ Current اللحظي (تنظيف التكرار ثم الإضافة)...")
        with engine.connect() as conn:
            for _, row in final_current.iterrows():
                conn.execute(
                    text("DELETE FROM current_weather WHERE city = :city AND time = :time"),
                    {"city": row['city'], "time": row['time']}
                )
            conn.commit()
        final_current.to_sql('current_weather', engine, if_exists='append', index=False)
        ...
        # تشغيل الإنذار وإرسال الإيميل الاستباقي
        check_for_alerts(final_daily)
        print(f"\n🎉 مبروك الـ 3 جداول بقوا لايف وجاهزين وتم تحديثهم بشكل تراكمي وبدون تكرار: {pd.Timestamp.now()}")
